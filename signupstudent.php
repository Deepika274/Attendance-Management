...
//echo "hey in try";
        ...
        if(empty($_POST['rollno'])){
          throw new Exception("Rollno cann't be empty.");
       }
        if(empty($_POST['phone'])){
         ...
         if(empty($_POST['dob'])){
            throw new Exception("DOB cann't be empty.");
         }
        ...
        $result2 = "update project.user set usertype ='student' where userid =('$_POST[uid]')";
        $stmt2 = $conn->prepare($result2);
        $stmt2->execute();
        $result3 = "insert into project.student (rollno , sname , dob , mobileno , gender) values ('$_POST[rollno]','$_POST[fname]','$_POST[dob]','$_POST[phone]','$_POST[gender]') ";
        $stmt3 = $conn->exec($result3);
        foreach($_POST["course"] as $course)
        {
          $result4 = "insert into project.enrolled values ('$_POST[rollno]' , '$course')";
...
<p> Student , Use their rollno as userid</p>
<div class="content">
<div class="row">
  <?php
...
<label for="input1" class="col-sm-3 control-label">Roll No</label>
          <div class="col-sm-7">
            <input type="text" name="rollno"  class="form-control" id="input1" placeholder="your roll number" />
          </div>
      </div>
...
<label for="input1" class="col-sm-3 control-label">Mobile No</label>
      ...
      <div class="form-group">
<label for="input1" class="col-sm-3 control-label">DOB</label>
          <div class="col-sm-7">
            <input type="date" name="dob"  class="form-control" id="input1" placeholder="dob" />
          </div>
      </div>
...
<br><br>
  <label for="courses">Choose all courses you are enrolled in:</label>
